...
public class AppmovilusuarioDao extends BaseDao<Appmovilusuario> {
	...
	public void mezclarLocal(Appmovilusuario obj)throws Exception{
		if(obj !=null){
			List<Appmovilusuario> lst = listar("LTRIM(RTRIM(t0.IDEMPRESA)) =? AND LTRIM(RTRIM(t0.IDAPPMOVIL))=?  AND LTRIM(RTRIM(t0.IDUSUARIO ))=?",obj.getIdempresa().trim(),
					obj.getIdappmovil().trim(),obj.getIdusuario().trim());
			if(lst.isEmpty())
				insertar(obj);
			else
				actualizar(obj);
